...
CREATE TABLE esquema2.Item (
    ...
    proveedor_id    VARCHAR(50) REFERENCES esquema2.Proveedor(nombre)
);

-- Creación de tabla Tiene
CREATE TABLE esquema2.Tiene (
    id_producto            SERIAL REFERENCES esquema2.Item(id_item),
    id_establecimiento     SERIAL REFERENCES esquema2.Instalacion(id_instalacion),
...
CREATE TABLE esquema2.Persona (
    ...
    DNI                     VARCHAR(9),
    lugar_trabajo           SERIAL REFERENCES esquema2.Instalacion(id_instalacion),
    nombre                  VARCHAR(30) NOT NULL,
