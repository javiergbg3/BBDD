...
CREATE TABLE Oficinas (
    codigo_oficina  NUMBER(4) PRIMARY KEY,
    direccion       VARCHAR(100) NOT NULL,
    telefono        VARCHAR(20) NOT NULL
);
...
CREATE TABLE Cuentas (
    numero_cuenta   NUMBER(20) PRIMARY KEY,
    IBAN            VARCHAR(4) NOT NULL,
    fecha_creacion  DATE NOT NULL,
    saldo_actual    DECIMAL(10,2) DEFAULT 0.00,
    sucursal        NUMBER(4) REFERENCES Oficinas(codigo_oficina),
...
CREATE TABLE Titulares (
    numero_cuenta   NUMBER(20) REFERENCES Cuentas(numero_cuenta),
    DNI_titular     VARCHAR(9) REFERENCES Clientes(DNI),
    interes         NUMBER(3,2) NOT NULL,  
    PRIMARY KEY (numero_cuenta, DNI_titular)
);

-- Creación de tabla Operaciones
CREATE TABLE Operaciones (
    id_operacion            NUMBER(10) PRIMARY KEY AUTO_INCREMENT,
